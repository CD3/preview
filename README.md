...
like in HTML or as a PDF. There are many situations when this is useful, here are a just a
...
Since creating `preview` I have several other use cases:

- View a PDF that is generated by a Python script when writing homework assignments with [pyAssignment](https://github.com/CD3/pyAssignment)
- View HTML slides generated from Markdown using [mdSlides](https://github.com/CD3/mdSlides)

The uses cases above are supported out of the box. In addition to these, it would (should?) be possible to write handlers that

- Run a Python script in a separate xterm window when it is saved.
- Run a bash script in a docker container on save.
- Use Python's Matplotlib library to plot data in a file and automatically update when the data file changes.
- Many more...
...
This script has gone through several iterations:

The first version worked well for viewing PDFs of markdown files and Latex snippets. Version 2 improved process handling. I had noticed that some orphaned processes
where being left behind for some file types and spend time figuring out how to make sure processes started by `preview` got killed/cleaned up when exiting. Version 3
improved the usability and expanded the number of supported file types.
...
is needed for preview to work is a `Makefile` that contains the following targets (it may define more that these, but these are the five that will be used).

- setup : [optional] `make setup` is ran before the view process is started.
- start : [required] `make start` is ran to start a process for viewing the preview (i.e. a PDF viewer or browser).
- refresh : [required] `make refresh` is ran every time the source file is modified (using `entr`).
- stop : [optional] `make stop` is ran when `preview` recieves a `SIGTERM` (i.e. ctrl-c).
- cleanup : [optional] `make cleanup` is ran after the processes started by `make start` terminates.
- check-dependencies: [optional] `make check-dependencies` is ran to test that the Makefile's dependencies are satisfied.
...
```
[[ -n $(make check-dependencies) ]] && exit 1
make setup
make start &
startPID=$!
...
kill $entrPID
```
Several handlers are provided and are good examples of how to write a new handler. These Makefiles are in the `dotpreview/` directory.
...
- `make check-dependencies` : This target check that the tools used by the Makefile are present. It should **not** output any text if the dependencies are satisfied. Output is used to indicate that there was an error so that you can print out a useful message if a dependency is missing. For example:
  ```
  check-dependencies:
  ...
  ```
  This target will print error messages if `gnuplot`, `sexpect`, or `zenity` are not found. `preview` will call `make check-dependencies` and print the output before exiting if any commands are missing.
